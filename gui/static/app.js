...
(function () {
  ...
  // --- data layer -------------------------------------------------------------
  // One page, two backends. "server" is the local app (gui/server.py), which
  // runs and stores audits. "static" is the published showcase (GitHub Pages):
  // bundled reports, read-only, plus reports a visitor opens, kept in their tab.
  ...
  var D = {
    ...
    init: function () {
      return api("GET", "api/meta").then(function (m) {
        D.mode = "server";
        return m;
      }).catch(function () {
    ...
    run: function (id, since) {
      ...
      copy.log = copy.log || [];
      copy.log_total = copy.log.length;
    ...
    importBody: function (body) {
      ...
      D.local[id] = {
        ...
        meta: {
          ...
          started_at: rep.audited_at || new Date().toISOString(), files: files,
  ...
  function setupForm() {
    if (!D.canRun()) {
      // The published showcase cannot crawl: say how to run the local app instead.
      var card = document.createElement("div");
      card.className = "card run-local";
      card.innerHTML = '<h2 class="card-title">Run your own audit</h2>' +
        ...
        '<pre class="cmd">' + h(localCommand()) + "</pre>";
      $("#new-audit").replaceWith(card);
  ...
  function renderEmpty() {
    ...
    if (!D.canRun()) {
      // Published showcase: no search box, the sample audits instead.
      ...
      box.innerHTML =
        ...
        '<div class="run-local-box"><strong>Run your own audit.</strong> This page is the published showcase, so it cannot crawl. ' +
        "The local app runs the same frozen engine and needs only Python. You can also drop any <code>audit_report.json</code> onto this page to read it here." +
        '<pre class="cmd">' + h(localCommand()) + "</pre></div>";
      return;
    }
    ...
    var foot = function () {
      ...
      $("#hero-foot").textContent = mode === "agent"
        ...
        : "Scripted engine: deterministic, no LLM, about a minute.";
    };
  ...
  function importFiles(files) {
    ...
    Promise.all(read).then(function (items) {
      ...
      toast(D.mode === "static" ? "Opened in this tab only. Nothing was uploaded." : "Report imported.");
      location.hash = "#/run/" + r.id;
      refreshHistory();
    }).catch(function (e) { toast(e.message); });
  }
  ...
  D.init().then(function (m) {
    ...
    $("#main").innerHTML = '<div class="banner">' + icon("info") + "<span>Cannot load the app: " + h(e.message) + "</span></div>";
  });
})();
